...
template <typename T>
typename T::result_type GetGlobalRandomNumberLockFree(T distribution) {
  return distribution(random_engine);
}
...
void printContainer(const Container& container, std::string sperator = " ",
                    const size_t new_line_per_element = 10) {
  size_t count = 0;
